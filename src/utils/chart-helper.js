export const constructData = (messages, includeInactiveDays) => {
  ...
  Object.values(messages).forEach(message => {
    ...
    if (action === "on") {
      ...
      let onFormattedDate = onDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' });
...
export const deconstructData = (data) => {
  ...
  data.forEach(entry => {
    ...
    highestUsagePerDay.push({ date: entry.date, total: entry.total, highestDevice: highestUsageDevice, highestUsage: entry[highestUsageDevice] });
  });
