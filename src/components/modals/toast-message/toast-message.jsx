...
export const ToastMessage = ({ setToastMessage, message }) => {
  ...
  const handleDragMove = (e) => {
    if (isDragging) {
      const clientX = e.type === 'mousemove' ? `${e.clientX}px` : `${e.touches[0].clientX}px`;
  ...
  return (
    ...
    >
      ...
      <Button className='nav-button' icon={<i className='fa-solid fa-xmark'></i>} onClick={handleClose} />
    </div>
  );
};